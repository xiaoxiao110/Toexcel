...
def getcase(title,get_dict):
    ...
    #计算一级模块数量
    get_module_first = len(get_case["topics"][0]["topics"])

    #计算二级模块数量
    get_module_second = len(get_case["topics"][0]["topics"][0]["topics"])

    #计算用例数
    get_case_num = len(get_case["topics"][0]["topics"][0]["topics"][0]["topics"])
        ...
        for j in range(0,num_module_first):
            ...
            for a in range(0,num_module_second):
                ...
                for b in range(0,num_case):
                    ...
                    """
                    #获取测试用例的优先级
                    get_priority = get_case["topics"][i]["topics"][j]["topics"][a]["topics"][b]["topics"][0]["makers"][0]
                    ...
                    print("操作步骤：",step)
                    #获取期望结果
                    hole_result = get_case[b]["topics"][1]["topics"][0]["title"]
